...
function getRawRanking( $table , $order ) { // ランキング情報を全て取得(登録順)
	...
	if($order === 'score') {
		...
		$sql = $pdo->prepare("SELECT * FROM $table ORDER BY time "); // 全データ取得　昇順
	}
	$sql->execute();
...
function getBestRecode( $table, $order, $flg=true ) { // 記録のベストを取得 flg=　true:MAX、 false:MIN
	$pdo = dbConection(); // データベースへの接続
	if( $flg ) {
		$sql = $pdo->prepare("SELECT MAX($order) FROM $table "); // MAX取得
	}
	else {
		$sql = $pdo->prepare("SELECT MIN($order) FROM $table "); // MIN取得
	}
	$sql->execute();
	$best = $sql->fetch(); // 取得データを変数に置き換え(リストで返ってくる)
	unset($pdo); // データベースからの切断
	//
	return $best[0];
}
...
// ランキングのデータ取得
if( isset($gMode) ) {
	$allRankingDATA = getRawRanking($gMode, $record); // 全データ取得
	$hiScore = getBestRecode( $gMode, 'score', true ) ?? 0; // ハイスコア取得(MAX)
	$bestTimeMax = getBestRecode( $gMode, 'time', true ) ?? 99999; // ベストタイム取得(MAX)
	$bestTimeMin = getBestRecode( $gMode, 'time', false ) ?? 99999; // ベストタイム取得(MIN)
}
else { 
	$allRankingDATA[0] = [
		'id' => '00000000',
...
<html lang="ja">
		...
		<table>
			...
			<tbody>
				...
				<tr <?php 
					...
					<td><?php 
						if( is_int($row['time']) ) {
							$min = floor($row['time']/(60*1000));
							$sec = floor(($row['time']/1000)%60);
							$ms = floor($row['time']%1000);
							echo sprintf('%02d',$min).':'.sprintf('%02d',$sec).'.'.sprintf('%02d',$ms);
						}
						else {
							echo 'Time';
						} ?></td>
	...
	<script>
		...
		// ?>;
		var hiScore_Ranking = <?php 
			print $hiScore ?? 0;
		?>;
		var bestTimeMax_Ranking = <?php 
			print $bestTimeMax ?? 9999999;
		?>;
		var bestTimeMin_Ranking = <?php 
			print $bestTimeMin ?? 60000;
		?>;
	</script>
</html>
